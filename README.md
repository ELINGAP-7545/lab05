...
Para este paquete de trabajo, debe descargarlo en su computador este repositorio , para ello debe clonar el mismo. Si no sabe cómo hacerlo revise la metodología de trabajo, donde se explica el proceso
...
2. Descargar la estructura propuesta de la  Unidad Aritmética de este git. Este proyecto cuenta con el archivo `alu.v` y, tiene la carpeta `src` que cuenta con las 5 carpetas de cada componente.
3. Implementar `alu.v` en la FPGA, y  comprobar el funcionamiento  de la suma y la multiplicación
4. Incluir el  HDL para le divisor al top alu.v, que se encuentra en la carpeta `src/divisor`  y, adicione los archivos e instanciar el bloque divisor.
